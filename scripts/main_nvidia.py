"""Nvidia sentiment analysis pipeline.
...
applied to Nvidia (NVDA) stock data.

All output files are prefixed with 'nvidia_' to keep results separate from
other analyses.

Usage (from project root, with .venv activated):
    python scripts/main_nvidia.py
"""
...
# ---------------------------------------------------------------------------
# Nvidia Configuration  (overrides the AAPL defaults in main.py)
# ---------------------------------------------------------------------------
...
# Input: Nvidia headlines produced by proquest_preprocessor.py
HISTORICAL_HEADLINES_PATH = (
    PROJECT_ROOT / "data" / "processed" / "wsj_nvidia_proquest.csv"
)

# No live-scrape fallback — if the ProQuest file is missing the pipeline stops.
HISTORICAL_HEADLINES_FALLBACK = None

# Outputs — every file is prefixed with 'nvidia_'
HISTORICAL_SENTIMENT_OUTPUT = (
    PROJECT_ROOT / "results" / "nvidia_historical_sentiment_analysis.csv"
)

# Experiment 1: Next-Day prediction
AGGREGATED_DATA_OUTPUT   = PROJECT_ROOT / "results" / "nvidia_sentiment_stock_dataset_nextday.csv"
MODEL_COMPARISON_RESULTS = PROJECT_ROOT / "results" / "nvidia_model_comparison_nextday.md"

# Experiment 2: Intraday prediction
INTRADAY_DATA_OUTPUT        = PROJECT_ROOT / "results" / "nvidia_sentiment_stock_dataset_intraday.csv"
INTRADAY_COMPARISON_RESULTS = PROJECT_ROOT / "results" / "nvidia_model_comparison_intraday.md"
...
def main() -> None:
    print("=" * 80)
    print("NVIDIA SENTIMENT ANALYSIS PIPELINE")
    ...
    if cache_valid:
        import pandas as pd
        print(
            f"\nLoading cached Nvidia sentiment scores from "
        ...
        if not source_path.exists():
            raise FileNotFoundError(
                f"Nvidia headlines file not found: {source_path}\n"
        ...
        historical_sentiment_df = analyze_headlines(
            ...
            tokenizer,
            model,
            source_label="historical Nvidia headlines",
        )
        if not historical_sentiment_df.empty:
            print("\nSentiment Analysis Summary (Nvidia Headlines):")
            print(
            ...
            print(
                f"\nCached Nvidia sentiment scores saved to "
                f"'{HISTORICAL_SENTIMENT_OUTPUT}'"
            )
    ...
    # ── Experiment 1: Next-Day Prediction ────────────────────────────────
    print("\n" + "=" * 80)
    print("EXPERIMENT 1 (NVIDIA): NEXT-DAY PREDICTION")
    ...
    if not merged_nextday.empty:
        ...
        cv_results_nextday = evaluate_models_with_cv(
            ...
            prediction_type="Next-Day (Nvidia)",
            n_splits=5,
        )
    ...
    # ── Experiment 2: Intraday Prediction ────────────────────────────────
    print("\n" + "=" * 80)
    print("EXPERIMENT 2 (NVIDIA): INTRADAY PREDICTION")
    ...
    if not merged_intraday.empty:
        ...
        cv_results_intraday = evaluate_models_with_cv(
            ...
            prediction_type="Intraday (Nvidia)",
            n_splits=5,
        ...
        train_comparison_models(
            ...
            technical_features=["overnight_gap", "open_vs_sma20", "rsi_change_lag1"],
            cm_suffix="nvidia_intraday",
        )
    else:
        print("Intraday dataset was not generated.")

    print("\n" + "=" * 80)
    print("NVIDIA PIPELINE COMPLETE")
    print("Results saved to results/nvidia_*")
    print("=" * 80)
